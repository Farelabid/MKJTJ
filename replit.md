...
- **Speedometer Gauge**: Replaces "PENDENGAR SAAT INI" box with SVG gauge featuring yellow-to-red gradient arc (gold → orange → tomato → crimson), scale marks 1.000-20.000 (Indonesian format with dots) representing actual listener count, animated dark gray needle (#4A4A4A) positioned based on current listener count, and center statistics showing current listeners, trend indicator with +/− signs, peak value, and percentage from peak. Needle calculation: angle = -180° + ((listeners - 1000) / 19000) × 200°.
